...
randomReplacement :: HiddenClockResetEnable dom => ReplacementFunc dom indexBits 2
randomReplacement _ _ _ = bitCoerce <$> toReplace
    where
    toReplace = register False $ not <$> toReplace

pseudoLRUReplacement 
    :: forall dom indexBits numWaysLog. (HiddenClockResetEnable dom, KnownNat indexBits, KnownNat numWaysLog, 1 <= numWaysLog) 
